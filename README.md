...
>  
*  当使用new（C++）或者malloc（C）分配内存时，被分配动态内存的区域，分配的内存块就位于堆区，堆空间不会跟栈空间一样手动释放内存，我们需要手动释放这些内存，否则可能导致内存泄漏。
...
在这个例子里面, &c 可以被定义，这意味着c是一个左值，在很多教科书里，$\text{lvalue}$ 有一个最符合表面含义的意思，便是，左值大多数指的是可以被用在赋值语句左侧的表达式，这也符合我们的认知，譬如```a = 5;```司空见惯但```5 = a;```就会被编译器报错一样。我们再来看一个例子:

```const int a;```

这里的a看起来是右值，因为常量不可被赋值。但这是对的吗？我们不妨采用取出a的地址:

```const int &b = a; ```

如果你跑这个程序的话，可以发现这是可以正常运行的，这说明a是个左值，只不过是个不可修改的左值。
...
在这几个例子里，m+n 和 -m 都是运算中的中间变量，在临时变量处理后迅速销毁；而数组的这个例子比较有趣，b是不可修改的左值，而数组中的具体元素譬如b[1]也是左值，而数组是右值。

我们同样可以给出很多关于左值和右值的例子，事实上，类的成员变量和返回引用的函数是左值，由于函数指针的存在，所有的函数都是左值，指针与引用也理所当然的是左值；而那些非静态成员函数，数组，this指针,lambda表达式，内置运算符的表达式都是右值。
...
# 什么是右值引用,$\text{std::move}$，$\text{std::forward}$？

我们来看一个例子：

> 假设你现在是一名程序员，你想要实现一个类的拷贝构造函数，而这个拷贝构造函数的传入参数又是一个数组(以$\text{std::vector}$为例)的时候，你通常会采取以下的写法：
> 
> ```
> T f(std::vector<T> data)
> ```
> 
> 但是你转念一想，在这个过程中，你等价于把你传进去的参数又拷贝了一次，这样的开销太大了，你想到了刚刚学过的引用，灵机一动，又为了避免函数修改你的$\text{std::vector}$,你把这个代码修改成:
> 
> ```
...
> 
> * 这个程序根本不可能运行!既然$\text{temp_array}$在函数体内进行了修改，所以它不能是个const左值引用；但如果函数参数改成非const，那么像Array的默认构造函数Array()作为右值就传不进这个函数了
> 
...
但俗话说的好：福无双至，祸不单行。虽然移动与拷贝并不局限于左右值，但我想要对一个左值进行移动时，我传入的参数需要是一个右值，于是我便需要一个将左值转换为右值的工具，那便是$\text{std::move}$。

![](figures/01.5.png)

可能这看起来比较奇怪，但是看了STL源代码后平平无奇。容易看出，实际上 $\text{move()}$所做的全部工作，不过是返回了通过 $\text{static_cast}$直接转型为右值引用后的实参。而 $\text{static_cast}$作为关键字，能够神奇的对表达式进行类型转换，而不创造新对象——相当于，在原对象上创造了一个新的使用视图，然后返回这个视图.
    
这告诉我们，值类型只是一种语法标记。我们前文对于是否拥有长期的地址来判断对象值类别的方式在右值引用的横空出世后不再有效。假如以这个标准考察$\text{std::move()}$的返回值，它们能够被移动，所以应该是右值。但它们又有地址——因为 static_cast不创造新对象，它们对应的还是原来的对象及其地址，只是视角变了——所以应该是左值。
    
在现代C++的语境下，左值和右值的语义进行了进一步扩张，有地址的值也可能可以被移动。而导致这样定义的幺蛾子正是像这样的既可以是左值也可以是右值的值类别的存在，它们的存在只是为了像$\text{static_cast<T&&>}$这样的东西可以被运行。这更加印证了我们的观点：**产生一个值的右值引用，或将它作为右值引用去使用，并不会对它本身产生任何影响。换句话说，右值引用只是个标志。只是让开发者可以便捷的用以右值引用为参数的函数重载，去实现如我们上面所做的，一些更高效的重载。**
...
```
这便是所谓完美引用的模版，这个模版的最大优点在于可以同时接收左值与右值；假如v是右值，传入参数时T被推导为实参的非引用类型；假如v是左值，传入参数时T被推导为实参的引用类型，我们来举个例子:
...
T&& && = T&&
```
用来保证上述的代码能够运行。同时函数体内部的$\text{std::forward}$保证了$v$的值类别在调用前后并不发生变化。假设把它去掉，那么函数体内部的$v$就是一个右值引用，其本身是一个左值，而传入时很可能没有左值的构造函数；而假如把$\text{std::forward}$改成$\text{std::move}$，那么假设传入的是一个左值，现在就变成了右值，这与原类中的函数想法背道而驰。
    
所以我们需要一个函数，让参数的值类别不发生变化，使得C++ 能够正确调用函数，这便是$\text{std::forward}$的STL代码。

> ![](figures/01.75.png)
    
至此，相信各位都对上文有了一定的了解，下面正式进入$\text{Smart Pointer}$.
...
简单来说，智能指针仅用于管理内存，用unique_ptr表达唯一所有权的指针，用shared_ptr表达共享所有权的指针，比起没有所有权的原始指针或引用，新的智能指针明确表达了生存期语义，可以有效地使各位程序员的大脑皮层更为光滑，实乃功不可没，那么下面就来具体了解一下智能指针，也是本次大作业所要完成的功能。
    
# $\text{Unique_ptr}$
    
顾名思义，之所以叫作$\text{unique_ptr}$，是因为其管理的对象具有独占性，即一个对象至多只能被一个指针管理。本质上，$\text{unique_ptr}$所要实现的最重要的功能是如何利用$\text{unique_ptr}$来对对象进行所有权的管理与转移，你可以让对象脱离$\text{unique_ptr}$的管理成为普通指针($\text{release}$函数的功能)，也要注意$\text{unique_ptr}$的独占性。当$\text{unique_ptr}$指针离开了作用域，使用了移动构造函数或者移动赋值，或者是给$\text{unique_ptr}$复制了一个新的对象或者空指针时，原有对象的$\text{unique_ptr}$都应该被删除。$\text{unique_ptr}$应当明确给出移动构造与移动赋值的具体实现方式来与拷贝进行区分。

![](figures/02.png)
    
为此，你需要构造的$\text{unique_ptr}$模版类的要求包括：
...
*  要求实现能够支持将普通指针转换成$\text{unique_ptr}$的构造函数,注意传入的参数的值类别形式：
    
     ```
    UniquePtr<int> ptr{new int{10}};
    ```
    
*  要求实现$\text{unique_ptr}$间的移动构造函数，并禁止拷贝构造函数的存在
    
     ```
     UniquePtr<int> ptr2{ptr1};//编译错误
    ```
    
*  在类外编写类似于$\text{std::make_unique}$的函数模版，要求能够支持形如
    ```
...
*    要求重载运算符```operator=```，实现$\text{unique_ptr}$间的移动赋值并禁止拷贝赋值
     ```
...
*    要求实现$\text{release}$函数用以返回一个指向原对象的普通指针，同时让对象脱离$\text{unique_ptr}$的管理。在此期间，$\text{unique_ptr}$类不删除原有指针。
     ```
...
在程序中，一个对象至多被一个指针管理的要求还是过于苛刻了，比如大一下的可持久化字典树大作业中，树结构的实现就需要用到$\text{shared_ptr}$.回观普通指针处理多个指针指向同一对象时，极易出现下面的问题:
    
![](figures/03.png)
    
当有多个指针指向同一个基础对象时，如果某个指针delete了该基础对象，对于其他指针来说，它们是无法感知的，此时则出现了悬垂指针，如果再对其他指针进行操作，则可能会导致$\text{Core Dump}$。

（$\text{Core Dump}$的原因：因为已经调用了delete，相当于已经将内存资源归还给了系统，如果有其他地方向系统申请资源时，系统则重新分配这块内存。此时有两种情况：原始的悬垂指针调用delete，系统检测到二次释放，直接$\text{Core Dump}$；或者是原始的悬垂指针对指针地址上的内存进行读、写操作，可能意外的改写了其他程序的内容，即“踩内存”，导致发生意想不到的情况）

解决方法也自然想到了**引用计数**：通过一块额外的内存，实现对原始内存的管理。每一个$\text{shared_ptr}$被新建时，其初始的引用计数为1。在每个$\text{shared_ptr}$对象的引用计数中，保存当前有多少个$\text{shared_ptr}$ 指针在管理同一个对象。当指向这个对象的某个智能指针被销毁后，引用计数-1，直到引用计数为0释放储存该对象的内存，这样就可以避免上述现象的产生。
    
$\text{shared_ptr}$中你需要实现的函数比起$\text{unique_ptr}$多了一些，你需要实现:
...
*  要求实现能够支持将普通指针和$\text{unique_ptr}$转换成$\text{shared_ptr}$的构造函数,注意传入的参数的值类别形式：
...
*  要求实现$\text{shared_ptr}$间的移动构造函数与拷贝构造函数
    
     ```
     SharedPtr<int> ptr2{ptr1};//编译成功
    ```
    
*  在类外编写类似于$\text{std::make_shared}$的函数模版，要求能够支持形如
    ```
...
*    要求重载运算符```operator=```，实现$\text{shared_ptr}$间的移动赋值与拷贝赋值
     ```
...
*    要求实现```use_count()```函数用以返回指向当前的对象的所有$\text{shared_ptr}$实例数
     ```
...
在实际应用中，我们很容易建构出上述引用计数的问题，考虑两个共享指针的$\text{shared_ptr}$对象（这类情况很容易出现，考虑双向链表或者有环图），不妨记其为$A$和$B$，其当前的引用计数都是2，在析构时两者都减1，从而两者的引用计数都变成了1而不为0，没有被正常析构。现在留下了一个矛盾的处境：必须销毁A才能销毁B，而销毁B必须先销毁A，这便是所谓的循环引用。

![](figures/04.png)
    
为了解决这个问题，引入了所谓的$\text{weak_ptr}$来解决循环引用问题。$\text{weak_ptr}$引入了第二个引用计数，为了与上文$\text{shared_ptr}$的引用计数相区分，将前者称为弱引用计数$\text{weak_cnt}$，后者称为强引用计数$\text{shared_cnt}$，注意需要对两者进行同时维护；
    
$\text{shared_cnt}$初始值为1，表示指向当前的对象的所有$\text{shared_ptr}$实例的数量，其遵循以下法则：
* $\text{shared_ptr}$拷贝构造时，$\text{shared_cnt}$+1；
* $\text{shared_ptr}$拷贝赋值时，$\text{shared_cnt}$+1；
* $\text{shared_ptr}$析构时，$\text{shared_cnt}$-1；
    
$\text{weak_cnt}$初始值为0，表示其观测到的资源被使用情况，其遵循以下法则：
* $\text{weak_ptr}$拷贝构造时，$\text{weak_cnt}$+1；
* $\text{weak_ptr}$拷贝赋值时，$\text{weak_cnt}$+1；
* $\text{weak_ptr}$析构时，$\text{weak_cnt}$-1；
* 当$\text{shared_cnt}$减为0时，$\text{weak_ptr}$-1；
    
下面来具体考察$\text{weak_ptr}$是如何破除循环引用的，现在是$\text{shared_ptr}$与$\text{weak_ptr}$之间的循环引用：
    
![](figures/05.png)

回到析构时的情形，$\text{shared_ptr}$与$\text{weak_ptr}$的强引用计数都减一，触发$\text{shared_ptr}$的析构
    
![](figures/06.png)
    
$\text{shared_ptr}$析构后，$\text{weak_ptr}$的计数减一，从而触发了$\text{weak_ptr}$的析构，破解了循环引用。（注意，倘若这里的$\text{shared_ptr}$不止一个，当最后的$\text{shared_ptr}$被销毁时，$\text{weak_ptr}$就应该过期）

![](figures/07.png)


可以看到，两个引用计数大体上是独立的。其实$\text{weak_ptr}$不像是一个指针，$\text{weak_ptr}$不需要重载解引用运算符```operator*```与间接引用运算符```operator->```，它更像是对$\text{shared_ptr}$对象管理的资源的观察者，它不影响共享资源的生命周期,更无法获取原对象的所有权。除此之外，在实际应用中，为了防止$\text{weak_ptr}$引用的对象在某个时刻被销毁，则需要函数```expired()```来检测其是否需要被观测，如果已经被删除，则将$\text{weak_ptr}$提升为$\text{shared_ptr}$用来继续使用其观察的资源，在这个过程中会触发强引用计数$\text{shared_cnt}$的计数(```lock()```函数)。
    
为此，你需要构造的$\text{weak_ptr}$模版类的要求包括：
...
*  要求实现能够支持将$\text{shared_ptr}$转换成$\text{weak_ptr}$的构造函数,注意传入的参数的值类别形式：
...
*  要求实现移动构造函数与拷贝构造函数，注意在拷贝构造中可以通过$\text{shared_ptr}$构造$\text{weak_ptr}$
...
*    要求重载运算符```operator=```，实现移动赋值与拷贝赋值，注意在拷贝赋值中可以将$\text{shared_ptr}$赋给$\text{weak_ptr}$
     ```
     WeakPtr<int> wp1, wp2;
     SharedPtr<int> sp{new int{10}};
     wp1 = sp;//编译成功
...
*    要求实现```lock()```函数用以转换$\text{weak_ptr}$为$\text{shared_ptr}$。如果对象已被删除，则返回一个空的$\text{shared_ptr}$。
      ```
      WeakPtr<int> wp{sp};
      ...
      ```
*    要求实现```expired()```函数用以检查$\text{weak_ptr}$所指向的对象是否已被删除，返回```true```表示被删除，返回```false```表示没有被删除
      ```
      WeakPtr<int> wp{sp};
      if(!wp.expired()) {
          SharedPtr<int> sp2 = wp.lock();
...
*    要求实现```swap()```函数，交换两个$\text{weak_ptr}$对象
     ```
...
1. 在完成本次大作业时，请对三个智能指针全部有所了解之后再开始设计，以避免不必要的局部修改或整体重构(尤其是$\text{shared_ptr}$与$\text{weak_ptr}$)
   
2. 注意函数实现中的细节，譬如$\text{const}$，$\text{noexpect}$等，这些都有可能成为$\text{Code Review}$中考察的对象。
